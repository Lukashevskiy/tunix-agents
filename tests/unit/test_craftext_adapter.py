...
def test_caged_adapter_has_identical_transition_contract() -> None:
    adapter = CagedCrafTextAdapter(TinyCrafText(terminal_timestep=1, caged=True), params=object(), action_count=3)
...
def test_adapter_rejects_a_vendor_action_mask_with_dynamic_wrong_shape() -> None:
    ...
    adapter = CrafTextAdapter(BadMaskEnvironment(terminal_timestep=1), params=object(), action_count=3)

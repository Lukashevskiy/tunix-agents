"""Batch Gemma/Tunix sampler boundary tests without local model weights."""

from __future__ import annotations

from types import SimpleNamespace

import pytest

from tunix_craftext.llm import LlmRequest
from tunix_craftext.prompts import ActionCatalog, RenderedPrompt
from tunix_craftext.tunix_adapter import GEMMA_ACTION_SYSTEM_PROMPT, GemmaTunixBackend


class _Tokenizer:
    def apply_chat_template(self, messages, *, add_generation_prompt, tokenize):
        del add_generation_prompt, tokenize
        return "|".join(f"{message['role']}:{message['content']}" for message in messages)

    def encode(self, text):
        return list(range(len(text.split())))

    def bos_id(self):
        return 0

    def pad_id(self):
        return 0


class _Sampler:
    def __init__(self) -> None:
        self.calls: list[dict[str, object]] = []

    def __call__(self, input_strings, **kwargs):
        self.calls.append({"input_strings": input_strings, **kwargs})
        return SimpleNamespace(
            text=[f"<action>WAIT</action>{index}" for index, _ in enumerate(input_strings)],
            logprobs=[[-0.4], [-0.6]],
            tokens=[[30], [40]],
            padded_prompt_tokens=[[5, 0], [6, 0]],
        )


def _backend() -> tuple[GemmaTunixBackend, _Sampler]:
    backend = GemmaTunixBackend.__new__(GemmaTunixBackend)
    sampler = _Sampler()
    backend._tokenizer = _Tokenizer()
    backend._sampler = sampler
    backend._cache_size = 128
    backend._seed = 13
    return backend, sampler


def _request(text: str, *, max_new_tokens: int = 4) -> LlmRequest:
    return LlmRequest(RenderedPrompt(text, ActionCatalog(("WAIT",)), "base"), max_new_tokens)


def test_gemma_backend_completes_a_prompt_batch_in_one_sampler_call() -> None:
    """Gemma chat prompts and provenance stay aligned with request ordering."""
    backend, sampler = _backend()

    responses = backend.complete_batch((_request("first"), _request("second")))

    assert len(sampler.calls) == 1
    assert sampler.calls[0]["input_strings"] == [
        f"user:{GEMMA_ACTION_SYSTEM_PROMPT}\n\nfirst",
        f"user:{GEMMA_ACTION_SYSTEM_PROMPT}\n\nsecond",
    ]
    assert [response.raw_text for response in responses] == [
        "<action>WAIT</action>0",
        "<action>WAIT</action>1",
    ]
    assert [response.backend for response in responses] == [
        "tunix-single-device:Gemma",
        "tunix-single-device:Gemma",
    ]
    assert [response.token_ids for response in responses] == [(30,), (40,)]
    assert [response.prompt_token_ids for response in responses] == [(5,), (6,)]


def test_gemma_batch_rejects_nonuniform_generation_contract() -> None:
    """One Tunix sampler invocation has one static generation length."""
    backend, _ = _backend()

    with pytest.raises(ValueError, match="max_new_tokens"):
        backend.complete_batch(
            (_request("first", max_new_tokens=2), _request("second", max_new_tokens=4))
        )

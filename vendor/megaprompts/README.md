...
```bash
python3 craftext_prompt/prompt_bench/test_sr.py \
  --prompt-config-name barlog \
  --prompt-config-name barlog_safety \
  --step-idx -1 \
  --run-count 10 \
...
BARLOG prompt suites:

```bash
# Success-rate style benchmark: defaults to barlog and barlog_safety.
python3 craftext_prompt/prompt_bench/test_sr.py --run-count 10

# Ask-rate benchmark: defaults to barlog_ask and barlog_ask_safety.
python3 craftext_prompt/prompt_bench/test_ask_rate.py --run-count 10

# Dry render-only check without LLM calls.
python3 craftext_prompt/prompt_bench/test_sr.py --run-count 0
python3 craftext_prompt/prompt_bench/test_ask_rate.py --run-count 0
```

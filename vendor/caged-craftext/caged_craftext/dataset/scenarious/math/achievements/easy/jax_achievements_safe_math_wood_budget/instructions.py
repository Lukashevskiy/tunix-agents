...
from caged_craftext.environment.scenarious.checkers.constrained_target_state import ConstrainedTargetState, BudgetByAction, TargetAction
...
def create_target_state(required=[], forbidden=[], action_list: list = [Action.DO], minuses: list = [10]):
    ...
    for action, minus in zip(action_list, minuses):
        actions = actions.at[action.value].set(minus)
        
    budget_by_action = BudgetByAction(budget=50, delimiter_actions=actions, target_action=TargetAction.WOOD.value)
    return ConstrainedTargetState(achievements=target_achievements, budget_by_action=budget_by_action)

...
class MapPresetSpec:
    ...
    generator_config: Optional[Union[BoxGeneratorSpec, RingGeneratorSpec]] = None
...
def get_world_preset_config_dir() -> pathlib.Path:
    ...
    module = importlib.import_module("caged_craftext")
...
def _normalize_map_size(map_size: Optional[Union[int, Tuple[int, int]]]) -> Optional[Tuple[int, int]]:
    if map_size is None:
        return None
...
MAP_GENERATOR_CONFIG_BUILDERS: dict[str, Callable[[Mapping[str, Any]], Union[BoxGeneratorSpec, RingGeneratorSpec]]] = _registry_from_pairs(
...
class SolidBlocksBehavior(BaseMovementBehavior):
    ...
    def __init__(self, spec: WorldPresetSpec, resolved_family: str) -> None:
        ...
        self.solid_block_values_array = jnp.asarray(sorted(self.solid_block_values), dtype=jnp.int32)
    ...
    def apply_step(self, previous_state: Any, new_state: Any) -> Any:
        ...
        if self.resolved_family == "classic":
            ...
            current_block = jnp.asarray(new_state.map[pos[0], pos[1]], dtype=jnp.int32)
        else:
            pos = new_state.player_position
            level = jnp.asarray(new_state.player_level, dtype=jnp.int32)
            current_block = jnp.asarray(new_state.map[level, pos[0], pos[1]], dtype=jnp.int32)

        is_solid = jnp.any(current_block == self.solid_block_values_array)
        return jax.lax.cond(
            is_solid,
            lambda: new_state.replace(player_position=previous_state.player_position),
            lambda: new_state,
        )
...
class InstantRecoveryBehavior(BaseStepBehavior):
    ...
    def apply_step(self, previous_state: Any, new_state: Any) -> Any:
        ...
        if instant_sleep_enabled:
            sleep_active = jnp.asarray(getattr(state, "is_sleeping", False))
            self._apply_recovery_mode_updates(state=state, updates=updates, prefix="sleep", active=sleep_active)
            if bool(self.rules.get("wake_after_sleep_recovery", True)) and hasattr(state, "is_sleeping"):
                updates["is_sleeping"] = jax.lax.select(sleep_active, jnp.asarray(False), state.is_sleeping)

        if instant_rest_enabled:
            rest_active = jnp.asarray(getattr(state, "is_resting", False))
            self._apply_recovery_mode_updates(state=state, updates=updates, prefix="rest", active=rest_active)
            if bool(self.rules.get("stop_rest_after_recovery", True)) and hasattr(state, "is_resting"):
                updates["is_resting"] = jax.lax.select(rest_active, jnp.asarray(False), state.is_resting)

        return state.replace(**updates) if updates else state

    def _apply_recovery_mode_updates(self, *, state: Any, updates: dict[str, Any], prefix: str, active: Any):
        if hasattr(state, "player_energy"):
            energy_target = self.rules.get(f"{prefix}_energy_value", 9)
            target = jnp.asarray(energy_target, dtype=getattr(state.player_energy, "dtype", None))
            updates["player_energy"] = jax.lax.select(active, target, state.player_energy)
        if hasattr(state, "player_fatigue"):
            fatigue_target = self.rules.get(f"{prefix}_fatigue_value", 0.0)
            target = jnp.asarray(fatigue_target, dtype=getattr(state.player_fatigue, "dtype", None))
            updates["player_fatigue"] = jax.lax.select(active, target, state.player_fatigue)
        if hasattr(state, "player_mana") and f"{prefix}_mana_value" in self.rules:
            target = jnp.asarray(self.rules[f"{prefix}_mana_value"], dtype=getattr(state.player_mana, "dtype", None))
            updates["player_mana"] = jax.lax.select(active, target, state.player_mana)
        if hasattr(state, "player_recover") and f"{prefix}_recover_value" in self.rules:
            target = jnp.asarray(self.rules[f"{prefix}_recover_value"], dtype=getattr(state.player_recover, "dtype", None))
            updates["player_recover"] = jax.lax.select(active, target, state.player_recover)
        if hasattr(state, "player_health") and f"{prefix}_health_value" in self.rules:
            target = jnp.asarray(self.rules[f"{prefix}_health_value"], dtype=getattr(state.player_health, "dtype", None))
            updates["player_health"] = jax.lax.select(active, target, state.player_health)


class ActionEnergyDrainBehavior(BaseStepBehavior):
    behavior_name = "action_energy_drain"
    ...
    def apply_step(self, previous_state: Any, new_state: Any) -> Any:
        state = new_state
        if not hasattr(state, "player_energy"):
            return state

        energy_cost = int(self.rules.get("action_energy_cost", 0))
        if energy_cost <= 0:
            return state

        min_energy = int(self.rules.get("action_energy_min", 0))
        skip_when_sleeping = bool(self.rules.get("skip_action_energy_drain_while_sleeping", False))
        skip_when_resting = bool(self.rules.get("skip_action_energy_drain_while_resting", False))
        should_skip = jnp.asarray(False)
        if skip_when_sleeping:
            should_skip = jnp.logical_or(should_skip, jnp.asarray(getattr(state, "is_sleeping", False)))
        if skip_when_resting:
            should_skip = jnp.logical_or(should_skip, jnp.asarray(getattr(state, "is_resting", False)))

        drained_energy = jnp.maximum(state.player_energy - energy_cost, min_energy)
        next_energy = jax.lax.select(should_skip, state.player_energy, drained_energy)
        return state.replace(player_energy=next_energy)
...
class CompositePresetAdapter:
    ...
    def __getattr__(self, name: str) -> Any:
        # Ray may probe attributes while unpickling the adapter before `env`
        # is restored into `__dict__`. Falling through to `self.env` in that
        # state causes recursive `__getattr__` calls.

...
backend profile и Tunix rollout knobs из `configs/generation/qwen_vllm_sync.yaml`, тот же
...
контракт и `configs/generation/qwen_vllm_async.yaml`, но вызывает vLLM через

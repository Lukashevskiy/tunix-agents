...
При restore мы передаём Orbax explicit `restore_args` из текущего `state_template`: для каждого
`jax.Array` фиксируются sharding, dtype и global shape. Это убирает warning вида
`Sharding info not provided when restoring` и не даёт Orbax молча брать sharding из checkpoint
metadata. Такое поведение важно для resume/preemption: checkpoint должен восстанавливаться в
текущую объявленную topology, а не случайно наследовать topology процесса, который его записал.

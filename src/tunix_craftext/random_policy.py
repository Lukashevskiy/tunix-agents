...
def sample_masked_actions(keys: jax.Array, action_mask: jax.Array) -> jax.Array:
    ...
    :param action_mask: Valid-action mask with shape ``[B, A]``.
    :returns: Integer action ids with shape ``[B]``.
    :raises ActionSamplingError: If mask/keys axes disagree. Use :func:`validate_action_mask`
        at the host boundary to reject empty rows before JIT execution.
    """
